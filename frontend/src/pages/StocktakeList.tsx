...
export interface ScmStocktakeRow {
  id: string;
...
export function StocktakeList() {
  ...
  const [params, setParams] = useStickyFilters("scm-stk", FILTER_KEYS);
  ...
  const [perPage, setPerPage] = useLocalStorage<number>("pp:scm-stk", 50);
  const { sort, sortParams, handleSortChange } = useServerSort(() => setPage(1));

  const list = useQuery<ListResp>(
    () =>
      api.get(
        `/api/scm-stocktakes${buildQuery({
          search,
          status,
          page,
          per_page: perPage,
  ...
  const columns: Column<ScmStocktakeRow>[] = [
    {
      key: "stocktake_number",
      label: "Stocktake #",
      alwaysVisible: true,
      render: (r) => <span className="font-mono text-xs font-medium">{r.stocktake_number}</span>,
      getValue: (r) => r.stocktake_number,
    },
    {
      key: "warehouse_code",
      label: "Warehouse",
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Stocktake"
        description="Count physical stock and reconcile on-hand quantities."
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => navigate("/scm/stocktakes/new")} icon={<Plus size={13} />}>
              New Stocktake
            </Button>
          </div>
        }
      ...
      <DataTable
        ...
        search={{
          ...
          placeholder: "Search stocktake number…",
        }}
        resetFilters={{
        ...
        error={list.error}
        emptyLabel="No stocktakes yet — click New Stocktake to count and reconcile stock."
        getRowKey={(r) => r.id}
        onRowClick={(r) => navigate(`/scm/stocktakes/${r.id}`)}
        serverSort
