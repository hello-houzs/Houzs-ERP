// ----------------------------------------------------------------------------
// PurchaseConsignmentReceiveNew — manual Create PC Receive at
// /purchase-consignment-receives/new. 1:1 clone of 2990s
// apps/backend/src/pages/PurchaseConsignmentReceiveNew.tsx (manual receipt, no
// parent PC Order). Pick supplier + warehouse, add lines, Create → inventory IN.
// SEAM playbook: ./consignment-purchase-queries; Strategy-2 plain-text lines;
...
import { useCreatePcReceive, useSupplierOptions, useWarehouseOptions, type NewPcrItem } from "./consignment-purchase-queries";
...
type DraftLine = { rid: string; materialCode: string; materialName: string; qtyReceived: number; unitPriceCenti: number };
const blankLine = (): DraftLine => ({ rid: `m${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, materialCode: "", materialName: "", qtyReceived: 1, unitPriceCenti: 0 });
...
export const PurchaseConsignmentReceiveNew = () => {
  ...
  const total = useMemo(() => lines.reduce((s, l) => s + l.qtyReceived * l.unitPriceCenti, 0), [lines]);
  const canSave = !!supplierId && lines.some((l) => l.materialCode.trim() && l.qtyReceived > 0);

  const onSave = async () => {
    if (!supplierId) return toast.error("Choose a supplier.");
    const realLines = lines.filter((l) => l.materialCode.trim() && l.qtyReceived > 0);
    if (realLines.length === 0) return toast.error("Add at least one item with qty > 0.");
    try {
      const items: NewPcrItem[] = realLines.map((l) => ({ materialKind: "mfg_product", materialCode: l.materialCode, materialName: l.materialName || l.materialCode, qtyReceived: l.qtyReceived, qtyAccepted: l.qtyReceived, unitPriceCenti: l.unitPriceCenti }));
      const res = await create.mutateAsync({ supplierId, warehouseId: warehouseId || null, receivedAt, deliveryNoteRef: deliveryNoteRef || undefined, notes: notes || undefined, items });
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div className={styles.titleBlock}>
          ...
          <div>
            <h1 className={styles.title}>New PC Receive</h1>
          </div>
        </div>
        <div className={styles.actions}>
          ...
          <Button variant="primary" onClick={onSave} disabled={saving || !canSave}>
            <Save {...ICON} />
            <span>{saving ? "Saving…" : "Create PC Receive"}</span>
          </Button>
        </div>
      </div>
      ...
      <section className={styles.card}>
        ...
        {lines.length === 0 ? (
          <div className={styles.cardBody}>
            <p className={styles.emptyRow}>Add an item to receive.</p>
          </div>
        ) : (
              ...
              {lines.map((l) => (
                <tr key={l.rid}>
                  ...
                  <td className={styles.tableRight}>
                    <input type="number" min={0} className={styles.fieldInput} style={{ width: 90, textAlign: "right" }} value={l.qtyReceived} onChange={(e) => setLine(l.rid, { qtyReceived: Math.max(0, Number(e.target.value) || 0) })} />
                  </td>
                  <td className={styles.tableRight}>
                    <InlineRmInput valueCenti={l.unitPriceCenti} onCommit={(centi) => setLine(l.rid, { unitPriceCenti: centi })} style={{ width: 110 }} />
                  </td>
                  <td className={styles.priceCell}>{fmtRm(l.qtyReceived * l.unitPriceCenti)}</td>
                  <td className={styles.tableRight}>
