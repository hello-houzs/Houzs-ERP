...
// Response shapes from GET /api/scm/purchase-consignment-receives/:id —
// snake_case, verbatim from the Hono route. It returns { grn, items }: the
// header carries supplier + parent PC Order joins and the consumption flags;
// each line carries qty_received / qty_accepted / qty_rejected, money, a
// server-resolved source_pco_number, the header received_at, and a downstream
// PC Return (PR) breakdown.
interface PcrHeader {
  id: string;
...
export function ScmPurchaseConsignmentReceiveDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const detail = useQuery<{ grn: PcrHeader; items: PcrItem[] }>(
    () => api.get(`${SCM}/purchase-consignment-receives/${id}`),
    [id],
  );

  const receive = detail.data?.grn;
  const items = detail.data?.items ?? null;
  const currency = receive?.currency ?? "MYR";

  if (detail.error) {
    return (
      <div>
        <BackLink onClick={() => navigate("/scm/purchase-consignment-receives")} />
        <EmptyState message="Failed to load purchase consignment receive" description={detail.error} />
      </div>
    );
  }

  // Live subtotal from the lines. The receive carries no tax, so
  // total = subtotal + tax_centi (tax is 0 in practice).
  const itemsSubtotal = (items ?? []).reduce((s, it) => s + (it.line_total_centi ?? 0), 0);
  const grandTotal = itemsSubtotal + (receive?.tax_centi ?? 0);

  const itemCols: Column<PcrItem>[] = [
    {
      key: "material_code",
      label: "Item Code",
    ...
    },
    {
      ...
      render: (it) =>
        it.source_pco_number ? (
          <span className="font-mono text-[12px]">{it.source_pco_number}</span>
        ) : (
          <span className="text-ink-muted">— (manual)</span>
        ),
      getValue: (it) => it.source_pco_number || "",
    },
    {
      key: "description",
      label: "Description",
    ...
    },
    {
      key: "delivery_date",
      label: "Delivery",
      render: (it) => fmtScmDate(it.delivery_date),
  ...
  return (
    <div>
      <BackLink onClick={() => navigate("/scm/purchase-consignment-receives")} />
      <PageHeader
        eyebrow={receive ? `Consignment Receive · ${receive.receive_number}` : "Consignment Receive"}
        title={receive?.supplier?.name ?? receive?.supplier?.code ?? (detail.loading ? "Loading…" : "Consignment Receive")}
      />
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Kpi
          label="Total"
          value={receive ? fmtCenti(grandTotal, currency) : "—"}
          loading={detail.loading}
        />
        <Kpi label="Received Date" value={receive ? fmtScmDate(receive.received_at) : "—"} loading={detail.loading} />
        <Kpi
          label="Status"
          value={receive ? statusLabel(receive.status) : "—"}
          loading={detail.loading}
        />
      </div>
      ...
      {receive && (
        <div className="mb-5 rounded-lg border border-border bg-surface p-5 shadow-stone">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h3 className="text-[10px] font-semibold uppercase tracking-brand text-ink-muted">Consignment Receive</h3>
            <span
              className={cn(
                "ml-auto inline-flex items-center rounded border px-2 py-0.5 text-[11px] font-semibold",
                scmStatusClasses(receive.status),
              )}
            >
              {statusLabel(receive.status)}
            </span>
          </div>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3 lg:grid-cols-4">
            <Info label="Receive No." value={receive.receive_number} mono />
            <Info label="Supplier" value={receive.supplier?.name ?? receive.supplier?.code} />
            <Info label="Transfer From (Order)" value={receive.purchase_consignment_order?.pc_number} mono />
            <Info label="Received Date" value={fmtScmDate(receive.received_at)} />
            <Info label="Delivery Note Ref" value={receive.delivery_note_ref} />
            <Info label="Currency" value={receive.currency} />
            <Info label="Subtotal" value={fmtCenti(receive.subtotal_centi, currency)} mono />
            <Info label="Total" value={fmtCenti(receive.total_centi, currency)} mono />
          </dl>
          {receive.notes && (
            <div className="mt-4 border-t border-border-subtle pt-3 text-[13px] text-ink-secondary">{receive.notes}</div>
          )}
        </div>
      )}
      ...
      <DataTable
        tableId="scm_pc_receive_items"
        columns={itemCols}
        rows={items}
        loading={detail.loading}
        getRowKey={(it) => it.id}
        emptyLabel="No line items on this consignment receive"
        exportName="purchase-consignment-receive-items"
      />
