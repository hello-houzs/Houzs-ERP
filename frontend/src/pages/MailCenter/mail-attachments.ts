...
//
// The POST /api/mail-center/compose + .../reply routes send SYNCHRONOUSLY via
// the Resend sender and BYPASS the email outbox, so each route owns its own cap
// (decoded-bytes 5 MB cap). These limits are MIRRORED here so the operator is
// rejected at the picker with the SAME error the server would return (project
// rule: frontend + backend validation unified). If you change a number here,
// change it on the backend (backend/src/lib/mail-attachments.ts) too.
//
...
export interface MailAttachmentValidationResult {
  ok: boolean;
  // Human-readable, English. Used verbatim in the inline FE error.
  error?: string;
...
// Validate the whole batch: count cap, per-extension allow-list, and a COMBINED
// decoded-size cap. Returns a single English error for the first violation.
