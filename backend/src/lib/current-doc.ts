...
//
// Ported VERBATIM from 2990s apps/api/src/lib/current-doc.ts (no seams — pure
// functions, no DB / framework coupling). Placed under backend/src/lib so the
// upcoming SCM slices (document-flow, SO/DO/SI lifecycle badges, …) reuse it.
// The PO slice does not call it yet; ported now per the program's
// "reusable libs -> backend/src/lib" rule.
//
