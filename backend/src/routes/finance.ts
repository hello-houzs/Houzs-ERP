 ...
 *             + project_finance.total_sales for QUICK-ENTRY projects
 *               (no sales_entries / sales_reports / income lines), by
 *               project end_date — see rawProjectQuickRevenue below
 ...
 *
 * Exhibition sales that flow through AutoCount are captured by
 * sales_orders, so project sales backed by entries/reports are NOT
 * summed here — they'd double-count the same revenue. The ONE
 * exception is the "Quick Total Sales" box (mig 088): projects where a
 * total was typed directly with NO sales order / entry / report. Those
 * are invisible to AutoCount, so we add only them — gated by the
 * NOT EXISTS clauses in rawProjectQuickRevenue so a project that has
 * any real sales record is never double-counted. Cancelled / archived
 * projects are excluded. Cancelled POs are excluded.
 *
...
function newBucket(key: string, label: string, start: string, endExclusive: string): Bucket {
  return {
    ...
    cost: 0,
    gross: 0,
    by_source: { sales_revenue: 0, project_quick_revenue: 0, project_cost: 0, service_cost: 0, po_cost: 0 },
  };
}
...
// Quick-entry project revenue: project_finance.total_sales for projects
// that have a typed total but NO sales record anywhere (no sales_entries,
// no project_sales_reports, no income project_finance_lines). Those are
// the "Quick Total Sales · no sales order needed" projects, invisible to
// AutoCount — so adding them can't double-count sales_orders. Bucketed by
// project end_date (fallback start_date). Cancelled / archived excluded.
async function rawProjectQuickRevenue(env: Env, start: string, end: string) {
  const rows = await env.DB.prepare(
    `SELECT COALESCE(p.end_date, p.start_date) AS d,
            COALESCE(pf.total_sales, 0) AS a
       FROM project_finance pf
       JOIN projects p ON p.id = pf.project_id
      WHERE COALESCE(pf.total_sales, 0) > 0
        AND p.archived_at IS NULL
        AND COALESCE(p.status, '') != 'cancelled'
        AND COALESCE(p.end_date, p.start_date) IS NOT NULL
        AND COALESCE(p.end_date, p.start_date) >= ?
        AND COALESCE(p.end_date, p.start_date) < ?
        AND NOT EXISTS (
          SELECT 1 FROM sales_entries se
           WHERE se.project_id = pf.project_id
             AND se.archived_at IS NULL
             AND se.status != 'void'
        )
        AND NOT EXISTS (
          SELECT 1 FROM project_sales_reports sr
           WHERE sr.project_id = pf.project_id
             AND sr.archived_at IS NULL
        )
        AND NOT EXISTS (
          SELECT 1 FROM project_finance_lines fl
           WHERE fl.project_id = pf.project_id
             AND fl.kind = 'income'
...
app.get("/pnl", requirePermission("projects.read"), async (c) => {
  ...
  const [salesRows, quickRevRows, projectRows, serviceRows, poRows] = await Promise.all([
    scope === "all" || scope === "sales"
      ? rawSales(c.env, overallStart, overallEnd)
      : Promise.resolve([]),
    scope === "all" || scope === "sales"
      ? rawProjectQuickRevenue(c.env, overallStart, overallEnd)
      : Promise.resolve([]),
  ...
  for (const r of quickRevRows) placeRow(buckets, r.d, r.a, "project_quick_revenue");
  ...
  for (const b of buckets) {
    b.revenue = b.by_source.sales_revenue + b.by_source.project_quick_revenue;
  ...
  const totals = buckets.reduce(
    (acc, b) => {
      ...
      acc.by_source.project_quick_revenue += b.by_source.project_quick_revenue;
