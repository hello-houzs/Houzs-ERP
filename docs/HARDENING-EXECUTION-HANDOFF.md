...
| #927 | `fix/so-cas-mandatory` / `021a3a99` | Mandatory Sales Order CAS plus transactional command/lease/allocation-outbox convergence | Restacked on #912. First real PG16 runs exposed composite-patch, JSON double-encoding and test-isolation defects; the branch now uses key-presence typed-row updates, preserves omitted fields, supports explicit null, binds JSON once and deterministically removes failure triggers. Both duplicate real-PG16 jobs pass all 10 integration cases; regular GitHub backend/frontend jobs, Claude Code and post-company-scope overlap review remain mandatory. Hard-blocked by #911's measured soak and #912. |
...
| `C:\Users\User\Desktop\hz-so-cas-mandatory` | `fix/so-cas-mandatory` / `021a3a99`; Draft PR #927 | Mandatory SO CAS, explicit JSONB/text[] codecs, durable allocation invalidation queue and retry worker | Restacked on #912 after independent review closed allocation lock/fence, PO/amendment CAS, replay and company-lock gaps. Both real PG16 jobs now pass 10/10 after protocol/SQL/test-isolation fixes; regular CI is still running. Claude Code and company-scope overlap review remain gates. PG migrations 0160-0162 precede D3 0163/0164. |
...
| `C:\Users\User\Desktop\hz-scale-real-schema` | `fix/scale-real-schema-fixture` / `79a12f6b` stacked on #913 | Production-shaped PostgreSQL scale fixture: two tenants, 100k orders/lines and 10k SKU/users per tenant; correctness/query-plan/latency evidence with transaction rollback | Author validation: scale contract 19/19, backend typecheck and SQLite smoke pass. Hard guards require loopback, exact disposable DB/ack and an empty catalog; no staging/prod connection was made. Independent review is active; real local PostgreSQL 100k evidence is still required before publish. |
...
- SO head `021a3a99` is published as stacked Draft PR #927 after independent review and restack onto #912. Real PG16 identified defects that local Windows could not execute; both duplicate PG16 jobs now pass all 10 cases. Regular CI, Claude Code and a post-company-scope overlap review remain mandatory. Never use superseded heads `3c03f4fb`, `8baa8226`, `e2b781bc`, `62caced2`, `19009edd`, `93e77260`, `6d81c9f3`, `92f1795c` or `c2119893`.
