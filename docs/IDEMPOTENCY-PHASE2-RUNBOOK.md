...
browser tab that is already open when the Worker deploys. Those tabs run the
previous JS bundle, never send a version, and would all receive 428 on their
next Save at the same moment.

`SO_CAS_GRACE_UNTIL` is the bounded window that prevents that.

| value | behaviour |
| --- | --- |
| unset (default) | strict: a missing `version` is 428 immediately |
| ISO-8601 instant in the FUTURE | a request that omits `version` is accepted with the pre-CAS last-writer-wins semantics |
| ISO-8601 instant in the PAST | strict again, automatically, with no deploy |

A STALE version is 409 in every case. The window only covers clients that cannot
speak the protocol at all — never a client that spoke it and lost.

Cutover:

1. Immediately before deploying, set the variable to deploy time + 30 minutes.
   ```
   npx wrangler secret put SO_CAS_GRACE_UNTIL   # e.g. 2026-07-22T14:30:00Z
   ```
2. Deploy. Existing tabs keep saving with the old semantics for 30 minutes.
3. After the instant passes the Worker is strict again on its own. Delete the
   variable at the next convenient deploy so the steady state is the default.

If the window is skipped entirely, expect every operator who is mid-edit at the
deploy moment to hit one 428 and to need a page refresh.

## Stock-allocation queue — clearing a dead letter (PR #927)

`scm.stock_allocation_recompute_queue` parks in `state='DEAD'` after 10
consecutive HARD failures. While it is parked the cron logs
`[cron so-allocation] DEAD LETTER …` at error level on every five-minute sweep
and performs no recompute. This is deliberate: an automatically-revived job is
what previously spun invisibly.

Diagnose first — `last_error` and `attempts` are the whole story:

```sql
SELECT state, attempts, deferrals, last_error, dead_lettered_at, requested_at, reason
FROM scm.stock_allocation_recompute_queue
WHERE job_key = 'GLOBAL';
```

Fix the underlying cause, then release the job:

```sql
UPDATE scm.stock_allocation_recompute_queue
SET state = 'PENDING', attempts = 0, deferrals = 0,
    last_error = NULL, dead_lettered_at = NULL, next_attempt_at = NULL,
    locked_by = NULL, locked_until = NULL
WHERE job_key = 'GLOBAL';
```

`deferrals` is a soft counter (an SO held by a human editor, or a concurrent
recompute). A high `deferrals` with `attempts = 0` is a busy shop, not a fault.
