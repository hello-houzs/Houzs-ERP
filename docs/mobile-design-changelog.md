...
### Service Case — shared stage pipeline + resolution routing (one logic layer) (v181)
73. **"One shared logic layer" — Phase 2 (Service Case): the stage routing rule now lives once.** `MobileServiceCase` reimplemented the 7-stage pipeline inline and, crucially, ignored the rule desktop enforced in `getActiveStages`: an **internal-resolution** case (own field service / return visit) skips the two supplier-only stages (Supplier Pickup + Item Ready). Consequences on mobile: the Advance button + Change-to select offered those stages (mis-routing an own-team case into them), and the progress bar / "N/7" counter used the wrong denominator. Refactor (frontend-only, no backend/migration): new PURE module `vendor/scm/lib/assr/stages.ts` — canonical `ASSR_STAGES` table + `resolutionRoute` / `isStageActive` / `activeAssrStages`. Desktop `getActiveStages` now delegates to the shared `isStageActive`; mobile derives an `activeStages` list from it and drives the **list-card mini bars, Stage-tab progress + phase tiles, Change-to select, Info accordion, and Advance button** off it — so an internal case now shows a clean **N/5** pipeline with no supplier stages. **Also fixed in passing:** mobile service-case writes invalidated `["mobile-assr-list"]`, which is not a prefix of the real list key `["mobile-assr-list-paged", …]`, so the list stayed stale until `staleTime`; corrected to the real prefix. **Intentionally deferred:** `ServiceProgressTracker` (uses server `stage_history.skipped`, a separate skip mechanism) + `MyCases` still keep their own copies of the stage table — dedup later; no correctness impact. *(Design-dir `.tsx` prototypes not overwritten; this changelog is the record.)*

### Sales Order — Phase 3a: stale-list fix + payment-map convergence (v182)
74. **New/Edit SO Phase 3a (correctness + dedup, zero behavior change).** Two fixes: (1) **Stale-list bug** — mobile SO create/edit (`MobileNewSO`), POD (`MobilePOD`), delivery (`MobileDeliveryPlanning`), and the app-shell post-scan refresh (`MobileApp`) all invalidated **dead** query keys `["mobile-so-list"]` / `["mobile-so-detail"]` that no query reads (the real keys are `["mobile-so-list-paged", …]` and `["mfg-sales-order-detail", docNo]`), so a freshly created/edited SO didn't show until staleTime/refocus. Desktop had the same class — `useCreateMfgSalesOrder` invalidated `['mfg-sales-orders']` while the live V2 list reads `['mfg-sales-orders-paged', …]`; now invalidates both. (2) **Payment method-code map** — `MobileNewSO` and `MobileSODetail` each hardcoded their own label→enum map; both now call the shared `paymentMethodCodeForValue` (`vendor/scm/lib/payment-methods.ts`) so the write-path codes can't drift. **Deferred to Phase 3b:** shared SO-form validation module (mobile is missing the past-date / processing>delivery guards and silently drops a slip-less payment — a money bug), shared create-payload builder + create hook, salesperson/venue/customer-type default helpers. **Deferred to 3c:** mobile `LineCard` → shared `SoLineCard`. *(Design-dir `.tsx` prototypes not overwritten; this changelog is the record.)*

### Sales Order — Phase 3b: shared save-time guards (money bug + date sanity) (v183)
75. **New/Edit SO Phase 3b — shared validation, closes a money bug.** New pure module `vendor/scm/lib/so-form-validate.ts` (`soDateGuardError` + `soSliplessPaymentError`), consumed by BOTH desktop `SalesOrderNew` and mobile `MobileNewSO`. (1) **Money bug** — mobile `recordSlipBackedPayments` only POSTs payment rows that carry a `slipSession`; a row with an amount but no slip uploaded was **silently dropped** and the payment never posted (SO showed unpaid). Desktop already blocked this (Spec D4); mobile now does too via the shared guard (guards only NEW rows in `pays`; recorded payments in `existingPays` are read-only and untouched). (2) **Date sanity** — mobile was missing desktop's past-date and processing>delivery guards; `soDateGuardError` now enforces set-together / not-past / processing≤delivery on both (draft skips the both-or-neither rule, matching mobile). Desktop's inline guards were replaced by the shared calls with identical messages → no desktop behavior change. **Deferred to 3c:** payment method-sub-field guard convergence (structural coupling), `deriveProcessingDate` extraction, salesperson/venue/customer-type default helpers, mobile `LineCard` → shared `SoLineCard`, mobile phone composition (hard `+60` prefix mangles international numbers). *(Design-dir `.tsx` prototypes not overwritten; this changelog is the record.)*
