...
else under `/api/*` requires a valid bearer session, then `companyContext`, then
principal/company/payload-bound `idempotency` (opt-in via `Idempotency-Key`).
**Gate** column = the per-route gate on top of that baseline.
...
(`dbInject` PG shim + `withPgDb`) · `auth` (session → user + RBAC) · `companyContext`
(active/allowed company) · `idempotency` (opt-in, principal/company/payload-bound replay)
· `rateLimit` (KV brute-force speed bump) · `caseTrack` (customer portal
